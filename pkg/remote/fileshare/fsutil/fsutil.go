...
func GetParentPathString(hostAndPath string) string {
	...
	t := hostAndPath[:lastSlash+1]
	if strings.HasSuffix(t, "walrus:////") {
		t = t[:len(t)-1]
	}
...
func GetEndingPart(path string) string {
	if path == "" || path == fspath.Separator {
		return ""
	}

	// Remove trailing slash if present
	if strings.HasSuffix(path, fspath.Separator) {
		path = path[:len(path)-1]
	}

	lastSlash := strings.LastIndex(path, fspath.Separator)
	if lastSlash <= 0 {
		return ""
	}
